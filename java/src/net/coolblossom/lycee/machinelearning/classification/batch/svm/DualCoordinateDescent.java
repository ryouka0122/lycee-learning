...
public class DualCoordinateDescent extends SVMOptimizer {
	...
	public double[] optimize(List<DataSet> dataSetList) {
		...
		List<Integer> dataIndex = Arrays.asList(Stream.iterate(0, n->n+1).limit(T).toArray(Integer[]::new));
	...
	private void debug(double[] a) {
		int nSV=0;
		for(double v : a) if(v>0 && v<softMarginPermit) nSV++;
