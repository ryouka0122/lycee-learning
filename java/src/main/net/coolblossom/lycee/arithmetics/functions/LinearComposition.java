...
public class LinearComposition extends Functor {
	...
	public double calc(double[] x) {
		if (this.parameters.length != x.length) {
			throw new IllegalArgumentException(new StringBuffer().append("引数の要素数が異なります[excepted:")
					.append(this.parameters.length).append("/actual:").append(x.length).append("]").toString());
		}
		double result = 0.0;
